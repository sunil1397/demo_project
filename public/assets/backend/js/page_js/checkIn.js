...
//     });
// }
var PurchaseOrderTable = $("#CheckInList")
  .DataTable({
    dom:
      "<'row'<'col-sm-12 col-md-2'l><'col-sm-12 col-md-4'f><'col-sm-12 col-md-6'<'toolbar'>>>" +
      "<'row'<'col-sm-12'tr>>" +
    ...
    ajax: {
      ...
      url: base_url + "/list-purchase-order-managements",
      type: "POST",
      data: function (data) {
...
var SavePurchaseOrderList = $("#listCheckIn")
  .DataTable({
    dom:
      "<'row'<'col-sm-12 col-md-2'l><'col-sm-12 col-md-4'f><'col-sm-12 col-md-6'<'toolbar'>>>" +
      "<'row'<'col-sm-12'tr>>" +
    ...
    columns: [
      {
        data: "check_in_details_id",
        name: "check_in_details_id",
        orderable: true,
        searchable: false,
      },
      {
...
$("div.toolbar").html(
  '<button type="button" aria-haspopup="true" onclick="addCheckIn()" aria-expanded="false" class="btn-shadow btn btn-info" title="Add new"><span class="btn-icon-wrapper pr-2 opacity-7"><i class="fa fa-plus fa-w-20"></i></span>Check In</button>'
);

$("#SavePurchaseOrderList_wrapper div.toolbar").html(
  '<button type="button" aria-haspopup="true" onclick="addCheckIn()" aria-expanded="false" class="btn-shadow btn btn-info" title="Add new"><span class="btn-icon-wrapper pr-2 opacity-7"><i class="fa fa-plus fa-w-20"></i></span>Check In</button>'
);
      ...
      submitHandler: function () {
        ...
        if (last_tr.length > 0) {
          ...
          $(last_tr).each(function () {
            if (
              $(this).val() == "" &&
              !$(this).hasClass("good-quantity") &&
              !$(this).hasClass("bad-quantity") &&
              !$(this).hasClass("shortage-quantity") &&
              !$(this).hasClass("supplier-id") &&
              !$(this).hasClass("excess-quantity")
            ) {
              x = 1;
            }
          });
              ...
              success: function (res) {
                ...
                if (res["status"]) {
                  ...
                  swal({
                    ...
                    $("#CommonModal").modal("hide");
                    window.location.href = base_url + "/" + "check-in-process";
                  });
                } else {
                  hideLoader();
                  swal("Opps!", res["msg"], "error");
...
$(document).on("click", "#get_order_details", function () {
  ...
  $.ajax({
    ...
    success: function (res) {
      ...
      if (res["status"]) {
        ...
        a.href = base_url + "/" + res["url"];
        a.download = res["barcode_number"] + ".png";
...
$("body").on("click", "#CreateOrder", function () {
  ...
  if (estimated_delivery_date == "") {
    ...
    $(last_tr).each(function () {
      if (
        $(this).val() == "" &&
        !$(this).hasClass("entry-product-name") &&
        !$(this).hasClass("category_name") &&
        !$(this).hasClass("gst") &&
        !$(this).hasClass("previous_lc_price")
      ) {
        x = 1;
      }
    });
  ...
  if (estimated_delivery_date == "") {
    ...
    $(last_tr).each(function () {
      if (
        ...
        !$(this).hasClass("entry-product-name") &&
        !$(this).hasClass("category_name") &&
        !$(this).hasClass("brand_name") &&
        !$(this).hasClass("uom_name")
      ) {
        x = 1;
      }
    });
...
// check Quanty 
// $(document).on('keyup', '#entryProductTbody .good-quantity, #entryProductTbody .bad-quantity , #entryProductTbody .shortage-quantity',function(element){
//   _this = $(this);
//   var good_qty = _this.parents('tr').find('.good-quantity').val();
//   var bad_qty = _this.parents('tr').find('.bad-quantity').val();
//   var shortage_quantity = _this.parents('tr').find('.shortage-quantity').val();
//   var total_quanty = parseInt(_this.parents('tr').find('.excess-quantity').val());
//   var check_qty = 0;
//   if( !isNaN(parseInt(good_qty) )){
//     check_qty += parseInt(good_qty);
//   }
//   if( !isNaN(parseInt(bad_qty))){
//     check_qty += parseInt(bad_qty);
//   }
//   if( !isNaN(parseInt(shortage_quantity))){
//     check_qty += parseInt(shortage_quantity);
//   }
//   console.log(check_qty);
//   if(total_quanty >= check_qty){

//   }else{
//     swal("Warning!", "Please Enter correct Quantity", "error");
//     _this.parents('tr').find('.good-quantity').val("0");
//     _this.parents('tr').find('.bad-quantity').val("0");
//     _this.parents('tr').find('.shortage-quantity').val("0");
//   }

//   // console.log(good_qty,bad_qty,shortage_quantity);
  
// });

$(document).on('keyup','#entryProductTbody .good-quantity',function(element){
  _this = $(this);
  var good_qty = parseInt(_this.parents('tr').find('.good-quantity').val());
  var bad_qty = parseInt(_this.parents('tr').find('.bad-quantity').val());
  var shortage_quantity = parseInt(_this.parents('tr').find('.shortage-quantity').val());
  var total_quanty = parseInt(_this.parents('tr').find('.excess-quantity').val());

  if((total_quanty < good_qty)){
    swal("Warning!", "Please Enter correct Quantity", "error");
    _this.parents('tr').find('.good-quantity').val("0");
    _this.parents('tr').find('.bad-quantity').val("0");
    _this.parents('tr').find('.shortage-quantity').val("0");
...
$(document).on('keyup','#entryProductTbody .bad-quantity, #entryProductTbody .shortage-quantity',function(element){
  _this = $(this);
  var good_qty = parseInt(_this.parents('tr').find('.good-quantity').val());
  var bad_qty = parseInt(_this.parents('tr').find('.bad-quantity').val());
  var shortage_quantity = parseInt(_this.parents('tr').find('.shortage-quantity').val());
  var all_quantity = parseInt(_this.parents('tr').find('.excess-quantity').val());
  var avl_qty = (all_quantity - good_qty);
  if(_this.hasClass('bad-quantity')){
    var bad_qty = _this.val();
    if(avl_qty>=bad_qty){
      _this.parents('tr').find('.shortage-quantity').val(avl_qty-bad_qty);
    }
    else{
      _this.val(0);
    }
  ...
  if(_this.hasClass('shortage-quantity')){
    var shortage_quantity = _this.val();
    if(avl_qty>=shortage_quantity){
      _this.parents('tr').find('.bad-quantity').val(avl_qty-shortage_quantity);
    }
    else{
      _this.val(0);
      ...
      success: function (res) {
        ...
        if (res["status"]) {
          ...
          for (var i = 0; i < res.data.length; i++) {
            ...
            var net_total =
              (parseFloat(mrp) * parseFloat(qty) * parseFloat(gst)) / 100 +
              parseFloat(mrp) * parseFloat(qty);
            net_total = net_total.toFixed(2);
            fetchData +=
              '<tr id="entryProductRow' +
              i +
              '"><td><input type="text" class="form-control entry-part-no" name="entry_part_no[]" autocomplete="off" value="' +
              res.data[i].pmpno +
              '"></td><td><input type="text" class="form-control entry-product-name" name="entry_product_name[]" readonly="readonly" value="' +
              res.data[i].part_name +
              '"><input type="hidden" class="form-control entry-product-id" name="entry_product[]" value="' +
              res.data[i].product_id +
              '"></td><td><input type="text" class="form-control category_name" name="category_name[]" readonly="readonly" value="' +
              res.data[i].category_name +
              '"><input type="hidden" class="form-control category_id" name="category_id[]" value="' +
              res.data[i].category_id +
              '"></td><td><input type="text" class="form-control gst" name="gst[]" readonly="readonly" value="' +
              res.data[i].vat +
              '"></td><td><input type="text" class="form-control mrp" name="mrp[]" readonly="readonly" value="' +
              res.data[i].mrp +
              '"></td><td><input type="number" class="form-control entry-product-quantity" name="entry_product_quantity[]" value="' +
              res.data[i].qty +
              '"></td><td class="total_price">' +
              net_total +
              '</td><td style="width: 12%;"> <button type="button" class="btn btn-danger btn-sm" title="Remove" onclick="removeProductEntry(' +
              i +
              ')"><i class="fa fa-window-close" aria-hidden="true"></i></button></td></tr>';
          }
          ...
          for (var i = 0; i < res.data.length; i++) {
            ...
            $("#entryProductTbody").append(
              '<tr id="entryProductRow' +
                ...
                '"></td><td><input type="number" class="form-control entry-product-quantity" name="entry_product_quantity[]" value="' +
                res.data[i].qty +
                '"></td><td><input type="text" class="form-control mrp" name="mrp[]" value="' +
                res.data[i].supplier_price +
                '"></td><td><input type="text" class="form-control gst" name="gst[]" readonly="readonly" value="' +
                res.data[i].vat +
                '"></td><td class="total_price">' +
                net_total +
...
// Delete Order
$(document).on("click", "a.delete-checkInList-order", function (e) {
  console.log("ok");
  var obj = $(this);
  var id = obj.data("id");
  swal({
    ...
    if (result.value) {
      $.ajax({
        ...
        url: base_url + "/delete-checkIn-list",
        type: "POST",
        data: { id: id },
        beforeSend: function () {
...
// View Order Details
$(document).on("click", "a.view-checkInList-order", function () {
  var obj = $(this);
  var id = obj.data("id");
  $.ajax({
    headers: {
    ...
    url: base_url + "/view-checkIn-details",
    type: "post",
    dataType: "JSON",
    data: { id: id },
    beforeSend: function () {
    ...
    success: function (res) {
      if (res["status"]) {
        hideLoader();
        $(".modal-title").text("").text("View Checking Details");
        $("#CommonModal").modal({
          backdrop: "static",
          keyboard: false,
        });
...
function checkAlreadyExistsProduct(_this, pmpno) {
  ...
  var last_tr = $("body #entryProductTbody tr")
    .not(parentTr)
    .find(".entry-part-no");
  var r = 0;
  $(last_tr).each(function () {
...
$("body")
  .off()
  .on("keyup", "#entryProductTbody .entry-part-no", function (element) {
    var _this = $(this);
    var supplier = $("#supplier").val();
      ...
      success: function (res) {
        ...
        if (res.data.length > 0) {
          ...
          _this
            .parents("tr")
            .find(".entry-part-no")
            .val(res.data[0].part_number);
          _this
            .parents("tr")
            .find(".entry-product-name")
            .val(res.data[0].part_name);
          ...
          _this
            .parents("tr")
            .find(".viewSupplierPrice")
            .text("MRP: " + res.data[0].cost_price);

          if (res.data[0].last_po_price > 0) {
            _this
              .parents("tr")
              .find(".viewLPP")
              .text("LPP: " + res.data[0].last_po_price);
            _this
              .parents("tr")
              .find(".previous_lc_price")
              .val(res.data[0].last_po_price);
          }
...
$("body").on(
  "keyup",
  "#entryProductTbody .tex, #entryProductTbody .entry-product-quantity, #entryProductTbody .SupplierPrice",
  function () {
    _this = $(this);
    ...
    $(".total_price").html(totalTaxAmount);
    var totalPrice = $("#totalPrice").val(totalTaxAmount);

    // console.log(_this.next('td.total_price'));
    // console.log("ok",_this.closest('td').next().find('.total_price'));
    // child('tr').find('.total_price').html(totalTaxAmount);
    // _this.child('tr').find('.total_price').html(totalTaxAmount);
  }
);
...
$("body").on(
  "keyup paste",
  "#entryProductTbody .entry-product-quantity, #entryProductTbody .mrp",
  function () {
    ...
    if (qty != "") {
      var mrp = 0;
      if (
        $(this).parents("tr").find(".mrp").val() > 0 &&
        $(this).parents("tr").find(".mrp").val() != NaN
      ) {
        mrp = $(this).parents("tr").find(".mrp").val();
      }
      var gst = 0;
      if (
        $(this).parents("tr").find(".gst").val() > 0 &&
        $(this).parents("tr").find(".gst").val() != NaN
      ) {
...
function final_calculation() {
  ...
  $("#entryProductTbody tr").each(function () {
    ...
    if (!isNaN(parseFloat($(this).find(".entry-product-quantity").val()))) {
      qty =
        $(this).find(".entry-product-quantity").val() == ""
          ? 0
          : parseFloat($(this).find(".entry-product-quantity").val());
    }
    var gst = "0";
...
function addExpenses() {
  ...
  $.ajax({
    ...
    success: function (res) {
      ...
      $("#CommonModal #budget_expenses_no").val(
        parseInt(budget_expenses_no) + 1
      );
    },
    error: function () {
      hideLoader();
...
function removeExpenses(id) {
  ...
  var ExpensesDetails = $("#CommonModal #ExpensesDetails").find(
    ".expenses-value"
  );
  $(ExpensesDetails).each(function () {
    ...
    if (expensesValue > 0) {
      CountexpensesValue =
        parseInt(CountexpensesValue) + parseInt(expensesValue);
    }
  });
  ...
  if (x == 1) {
    ...
    $.ajax({
      ...
      success: function (res) {
        if (res["status"]) {
          window.open(
            base_url + "/" + res.return_url + "?order_id=" + res.order_id
          );
        } else {
          swal("Warning!", res["msg"], "error");
        }
...
var ExcessPurchaseOrderList = $("#ExcessPurchaseOrderList")
  .DataTable({
    dom:
      "<'row'<'col-sm-12 col-md-2'l><'col-sm-12 col-md-4'f><'col-sm-12 col-md-6'<'toolbar'>>>" +
      "<'row'<'col-sm-12'tr>>" +
...
var DamagePurchaseOrderList = $("#DamagePurchaseOrderList")
  .DataTable({
    dom:
      "<'row'<'col-sm-12 col-md-2'l><'col-sm-12 col-md-4'f><'col-sm-12 col-md-6'<'toolbar'>>>" +
      "<'row'<'col-sm-12'tr>>" +
...
var ShortagePurchaseOrderList = $("#ShortagePurchaseOrderList")
  .DataTable({
    dom:
      "<'row'<'col-sm-12 col-md-2'l><'col-sm-12 col-md-4'f><'col-sm-12 col-md-6'<'toolbar'>>>" +
      "<'row'<'col-sm-12'tr>>" +
