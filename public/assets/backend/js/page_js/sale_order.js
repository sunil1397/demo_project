    var saleOrderTable = $('#sale_orders').DataTable({
        ...
        'columns': [
            ...
            {data: 'totalItems', name: 'totalItems', orderable: false, searchable: false},
    ...
    $('body').on('click', '.preview-multiple', function(){
        var customer = $('#client').val();
        if(customer == "") {
            swal("Warning!", "Please select customer", "error");
        }else {
            var file_data = $('#product_csv').prop('files')[0];
            if(file_data) {  
                ...
                $.ajax({
                    url: base_url+"/sales-order-preview", 
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    },
                    dataType: 'html',  
...
// // View Bulk Upload
// function bulk_upload_salesOrder(){
//   $.ajax({
//       url:base_url+"/bulk-upload-sales-order",
//       type:'get',
//       dataType:'html',
//       beforeSend:function(){
//           showLoader();
...
//           hideLoader();
//           // if(res['status']) {
//           $('.modal-title').text('').text("Bulk Upload Sales Order");
//           $("#CommonModal").modal({
//               backdrop: 'static',
//               keyboard: false
//           });
...
// Save Bulk Upload 
function bulk_save_sales_order() {
  $("#OrderPreviewModal").find("#previewOrderForm").validate({
      rules: {
      ...
      submitHandler: function() {
        ...
        if(x==1)
        {
          ...
          $.ajax({
              ...
              url:base_url+"/save-bulk-upload-sales-order",  
              type: "POST",
              data:  form_data,
              contentType: false,
              cache: false,
...
function subtotalCalculation(){
    var subTotal = 0;
    $('body').find('#order_row tr').each(function(){
...
function grandCalculation(){
    var grand_total = 0;
    $('body').find('#order_row tr').each(function(){
        if($(this).find('.row_total').html() != undefined)
        grand_total +=parseFloat($(this).find('.row_total').html());
    });
