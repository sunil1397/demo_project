    var purchaseOrderTable = $('#purchaseReturn').DataTable({
        ...
        "ajax": {
            ...
            "url": base_url+"/purchase-return-list",
            "type": "POST",
            'data': function(data){
        ...
        'columns': [
            ...
            {data: 'purchase_order_invoice_date', name: 'purchase_order_invoice_date', orderable: false, searchable: false},
            {data: 'supplier_id', name: 'supplier_id', orderable: false, searchable: false},
    ...
    $("#filter_supplier").on('keyup',function(){
    ...
    $('div.toolbar').html('<button type="button" aria-haspopup="true" id="add_order1_management" aria-expanded="false" class="btn-shadow btn btn-info" onclick="addPurchaseReturn()"><span class="btn-icon-wrapper pr-2 opacity-7" ><i class="fa fa-plus fa-w-20"></i></span>Add Purchase Return</button>');
...
function addPurchaseReturn() {
  $.ajax({
      ...
      url: base_url + "/add-purchase-return",
      type: "post",
      beforeSend: function () {
        showLoader();
      },
      success: function (res) {
        hideLoader();
        $(".modal-title").text("").text("Add Purchase Return");
        $("#CommonModal").modal({
          backdrop: "static",
          keyboard: false,
        });
    ...
    $("#CommonModal")
      ...
      .validate({
        ...
        submitHandler: function () {
              ...
              var r=0;
              $("#inviceDetailsTbody").find('tr').each(function(i){
                if($(this).find('input[type="checkbox"]').prop('checked') == true){
                  formData.append('allocated_row_id[]',$(this).find('input[type="checkbox"]:checked').val());
                  formData.append('category_id[]',$(this).find('input[type="text"]').val());
                  formData.append('purchase_invoice_date_per_order[]',$(this).find('input[type="text"]').val());
                  r=1;
                }
              });
              if(r==0)
              {
                swal("Warning!", "Please checked at least one row", "error");
                return;
              }
              $.ajax({
                headers: {
                ...
                url: base_url + "/save-purchase-return",
                type: "POST",
                data: formData,
                contentType: false,
                cache: false,
  ...
  function addPurchaseReturninvoice(){
    ...
    if (supplier == "") {
      ...
      $.ajax({
        ...
        url: base_url + "/get-purchase-details",
        type: "POST",
...
// $(document).on('submit', '#PurchaseReturnForm', function() {
//   console.log("Hello purchase Form");
//   return false;
//   var obj = $(this);
//   var id = obj.data("id");
