...
var departmentTable = $('#departmentList').DataTable({
    ...
    'columns': [
        {data: 'department_id', name: 'department_id', orderable: true, searchable: false},
...
$('div.toolbar').html('<button type="button" aria-haspopup="true" id="add_catagory" aria-expanded="false" class="btn-shadow btn btn-info" onclick="show_form()"><span class="btn-icon-wrapper pr-2 opacity-7" ><i class="fa fa-plus fa-w-20"></i></span>Add Department</button> <button type="button" aria-haspopup="true" id="add_catagory" aria-expanded="false" class="btn-shadow btn btn-info" onclick="ExportDepartmentTable()"><span class="btn-icon-wrapper pr-2 opacity-7" ><i class="fa fa-download fa-w-20"></i></span>Export</button>');
function ExportDepartmentTable() {
    var info = departmentTable.page.info();
    window.location.href = base_url+"/department-export?start="+info.start+"&end="+info.end;
}
// Add Department Form
function show_form(){
     $.ajax({
        ...
        success:function(res){
            ...
            hideLoader();
            // if(res['status']) {
            $('.modal-title').text('').text("Add Department");
            $("#CommonModal").modal({
                backdrop: 'static',
                keyboard: false
            });
...
function department_form() {
    $("#CommonModal").find("#departmentForm").validate({
        rules: {
            department: "required",
        },
...
// Delete Department 
$("body").on("click", "a.delete-department", function(e) {                   
    var obj = $(this);
    var id = obj.data("id");
    swal({
        title: "Are you sure?",
...
// Edit Department-->
$('body').on('click',"a.edit-department",function(){
    var id = $(this).data('id');
    $.ajax({
        headers: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        ...
        success:function(res){
            hideLoader();
            if(res["status"]) {
                $('.modal-title').text('').text("Update Department");
                $("#CommonModal").modal({
                    backdrop: 'static',
                    keyboard: false
                });
