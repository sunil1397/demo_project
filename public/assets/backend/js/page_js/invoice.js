// dataTable
var invoiceTable = $('#invoice').DataTable({
...
// <button type="button" aria-haspopup="true" id="add_item_management" aria-expanded="false" class="btn-shadow btn btn-info" onclick="show_form()"><span class="btn-icon-wrapper pr-2 opacity-7" ><i class="fa fa-plus fa-w-20"></i></span>Barcode Scanner</button> 
...
function ExportItemManagementTable() {
    ...
    window.location.href = base_url+"/invoice-export?start="+info.start+"&end="+info.end;
}

// View Invoice Details
$(document).on('click', 'a.view-invoice-order', function() {
  var obj = $(this);
  var id = obj.data("id");
  $.ajax({
      headers: {
      ...
      url:base_url+"/view-invoice-details",
      type:'post',
      dataType:'JSON',
      data: {id:id},
      beforeSend:function(){
      ...
      success:function(res){
          if(res['status']) {
              hideLoader();
              $('.modal-title').text('').text("Invoice Details");
              $("#CommonModal").modal({
                  backdrop: 'static',
                  keyboard: false
              });
...
$(document).on('click', '.download-invoice', function() {
    ...
    $.ajax({
        ...
        //     if(res['status']) {
        //         hideLoader();
        //         $('.modal-title').text('').text("Download invoice");
        //         $("#CommonModal").modal({
        //             backdrop: 'static',
        //             keyboard: false
        //         });
